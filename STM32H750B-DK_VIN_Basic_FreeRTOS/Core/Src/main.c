...
uint32_t LED_BlinkingBuffer[ 128 ];
osStaticThreadDef_t LED_BlinkingControlBlock;
const osThreadAttr_t LED_Blinking_attributes = {
  .name = "LED_Blinking",
  .cb_mem = &LED_BlinkingControlBlock,
  .cb_size = sizeof(LED_BlinkingControlBlock),
  .stack_mem = &LED_BlinkingBuffer[0],
  .stack_size = sizeof(LED_BlinkingBuffer),
  .priority = (osPriority_t) osPriorityLow,
};
/* Definitions for KEY_LED */
osThreadId_t KEY_LEDHandle;
uint32_t KEY_LEDBuffer[ 128 ];
osStaticThreadDef_t KEY_LEDControlBlock;
const osThreadAttr_t KEY_LED_attributes = {
  .name = "KEY_LED",
  .cb_mem = &KEY_LEDControlBlock,
  .cb_size = sizeof(KEY_LEDControlBlock),
  .stack_mem = &KEY_LEDBuffer[0],
  .stack_size = sizeof(KEY_LEDBuffer),
  .priority = (osPriority_t) osPriorityLow,
};
/* USER CODE BEGIN PV */
#define 	BUFSIZE 256  // Konstanta
char	SendBuffer[BUFSIZE];  // Polje znakov velikosti 256
int	Counter;             // Stevec
int KeyState=0;          // Celoštevična spremenljivka
...
/* Configure the peripherals common clocks */
  ...
  LED_BlinkingHandle = osThreadNew(StartTaskLED, NULL, &LED_Blinking_attributes);

  /* creation of KEY_LED */
  KEY_LEDHandle = osThreadNew(StartTaskKEYLED, NULL, &KEY_LED_attributes);
...
void StartDefaultTask(void *argument)
{
  ...
  for(;;)
  {
		snprintf(SendBuffer,BUFSIZE,"Hello World [%d]: Key:%d\n\r",Counter++,KeyState);  // Pripravi tekstovno sporočilo s stanjem v polje znakov - string
		HAL_UART_Transmit(&huart3,SendBuffer,strlen(SendBuffer),100); // Pošlji sporočilo s stanjem po USART povezavi v PC
...
/* USER CODE BEGIN Header_StartTaskKEYLED */
/**
* @brief Function implementing the KEY_LED thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_StartTaskKEYLED */
void StartTaskKEYLED(void *argument)
{
